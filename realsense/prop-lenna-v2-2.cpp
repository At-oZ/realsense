...
//
//    // 出力ファイルを開く
//    std::ofstream file("./images/lenna/prop-reconstruction/average-time-v2-2-note-interpolation.csv");
//
//    // ファイルが正しく開けたか確認
//    if (!file.is_open()) {
...
//                                    tmp_nz = static_cast<int>(floor(coef * tmp_zt + 0.5));
//
//                                    dx = tmp_pcd_x - (tmp_nx - 0.5 - px_width_img * 0.5) * img_pitch / gap * tmp_zt;
//                                    dy = tmp_pcd_y - (tmp_ny - 0.5 - px_width_img * 0.5) * img_pitch / gap * tmp_zt;
//
...
//                    // 表示画像の保存
//                    ostringstream stream;
//                    stream << "./images/lenna/prop-reconstruction/v2-2/prop-lenna-v2-2_ImgDisplay_NumPinhole" << num_pinhole << "_NxNy" << px_width_img << "_Nz" << num_z_level << "_pitchTimes" << static_cast<int>(ptimes) << "_subjectZ" << (int)subject_z << ".png";
//                    cv::String filename = stream.str();
//                    imwrite(filename, img_display);
//
//                    stream.str("");

...
//            double tileLeft = -tileSize / 6.0;
//            double tileRight = tileSize / 6.0;
//
//            // カメラ位置の計算
...
//
//                            // ピクセルインデックスの範囲を計算
//                            int xMinIdx = std::max(0, static_cast<int>(std::floor((visibleAreaMinX + tileSize / 6) / tilePixelSize)));
//                            int xMaxIdx = std::min(tileResolution - 1, static_cast<int>(std::floor((visibleAreaMaxX + tileSize / 6) / tilePixelSize)));
...
//                            {
//                                // ピクセルのx座標範囲を計算
//                                double xPixelMin = xIdx * tilePixelSize - tileSize / 6;
//                                double xPixelMax = (xIdx + 1) * tilePixelSize - tileSize / 6;
//
...
//
//                                    // ピクセル値を取得し、加重平均に加算
//                                    cv::Vec3b pixel = tileImageCenter.at<cv::Vec3b>(yIdx, xIdx);
...
//
//            // 画像の保存
//            std::string filename = "C:/Users/taw11/EvacuatedStorage/prop-reconstruction/ideal-v1-1/ideal-tileNotExpand_parrots_gridSize" + std::to_string(static_cast<int>(gridSize)) + "_zi" + std::to_string(static_cast<int>(tileZ)) + ".png";
//

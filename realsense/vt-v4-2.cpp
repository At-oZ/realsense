...
//}
//
//void insert_pixels(int WIDTH, int HEIGHT, int Wpin, int Hpin, int N, double g, double wp, double pd, double intv, int starty, int endy, int* img, cv::Mat img_display, double** val_z) {
//
//    int nx, ny;
//    int startv, startu;
...
//
//                    // cout << "horizontal dispality:" << static_cast<int>(floor((j + 0.5 - Wpin / 2) * (wp * g / (pd * val_z[k][l] * 1000)))) << endl;
//                    // cout << "vartical dispality:" << static_cast<int>(floor((i + 0.5 - Wpin / 2) * (wp * g / (pd * val_z[k][l] * 1000)))) << endl;
//
...
//    int* img;
//    img = (int*)malloc(sizeof(int) * (N * range) * (N * range) * 3);
//
//    // フレーム処理
...
//
//                    img[(ny * (N * range) + nx) * 3] = static_cast<int>(tmp_pcd3);
//                    img[(ny * (N * range) + nx) * 3 + 1] = static_cast<int>(tmp_pcd4);
//                    img[(ny * (N * range) + nx) * 3 + 2] = static_cast<int>(tmp_pcd5);
//
...
//
//        // insert_pixels(WIDTH, HEIGHT, Wpin, Hpin, N, g, wp, pd, intv, 0, Hpin, std::ref(img), std::ref(img_display), val_z);
//
//        int startRow, endRow;
//        for (int i = 0; i < numThreads; i++) {
//            startRow = i * rowsPerThread;
//            endRow = (i == numThreads - 1) ? Hpin : (i + 1) * rowsPerThread;
//            threads.emplace_back(insert_pixels, WIDTH, HEIGHT, Wpin, Hpin, N, g, wp, pd, intv, startRow, endRow, std::ref(img), std::ref(img_display), val_z);
//        }
//
//        for (auto& t : threads) {
...
//        stream << "mac_v4-2_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png"; // �����_�ȉ�2���Ő؂�̂�

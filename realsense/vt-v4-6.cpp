...
//}
//
//void insert_pixels(int starty, int endy, int* img, int** img_display, double** val_z, double** val_z2) {
//
//    int nx, ny;
//    int startv, startu;
//    // cout << "clear" << endl;
...
//                        ny -= N * (range - 1) * 0.5;
//
//                        if (val_z[l][k] < val_z2[startv + ny][startu + nx]) {
//
...
//                            val_z2[startv + ny][startu + nx] = val_z[l][k];
//                        }
//                    }
//                }
//            }
...
//    cv::Mat img_save = cv::Mat::zeros(cv::Size(N * range, N * range), CV_8UC3);
//
//    // 要素画像群
...
//
//    double** val_z2;
//    val_z2 = (double**)malloc(sizeof(double*) * ph_disp);
//    for (int i = 0; i < ph_disp; i++) {
//        val_z2[i] = (double*)malloc(sizeof(double) * pw_disp);
//    }
//
//    // CSVファイルのパス
...
//            threads.emplace_back(insert_pixels, startRow, endRow, std::ref(img), std::ref(img_display), std::ref(val_z), std::ref(val_z2));
//        }
//
//        for (auto& t : threads) {
...
//        for (int i = 0; i < N * range; i++) {
//            for (int j = 0; j < N * range; j++) {
//                img_save.at<cv::Vec3b>(i, j)[2] = static_cast<int>(floor(img[i * (N * range) + j] / (256 * 256)));
//                img_save.at<cv::Vec3b>(i, j)[1] = static_cast<int>(floor(img[i * (N * range) + j] / 256 - img_save.at<cv::Vec3b>(i, j)[2] * 256));
//                img_save.at<cv::Vec3b>(i, j)[0] = static_cast<int>(floor(img[i * (N * range) + j] - (img_save.at<cv::Vec3b>(i, j)[2] * 256 - img_save.at<cv::Vec3b>(i, j)[1]) * 256));
//            }
//        }
//
//        ostringstream stream;
//        stream << "./images/v4-6_center_img__g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//

...
//    const int image_size_px = 256;      // タイルに貼る画像の解像度（lennaの画像を直接みるときは256px, 表示系の再現の場合はdisplay_image_pxと同じ）
...
//            const double tile_pos = subz; // mm (lennaの画像を直接みるときはsubzと同じ, 表示系の再現の場合はギャップと同じ（4.0 * 160 / (double)nph mm）)

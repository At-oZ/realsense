...
//	Shader compose("PCSJ2025-v3-shader.vert", "PCSJ2025-v3-shader.frag");
//	GLint locInvZ = glGetUniformLocation(compose.ID, "uInvZ");
//	GLint locInvZo = glGetUniformLocation(compose.ID, "uInvZo");
//
//	// 点群データの生成
//	//------------------------------
//
...
//	}
//
//	GLuint baseOffsetTex = 0;
//	{
//		glGenTextures(1, &baseOffsetTex);
//		glBindTexture(GL_TEXTURE_2D, baseOffsetTex);
//		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
//		// GL_RG32F: (x,y) = baseOffset
...
//			threads.emplace_back(zerosBox, startRow, endRow, ref(red), ref(green), ref(blue), ref(numPoints));
//		}
//
//		for (auto& t : threads) {
...
//			startRow = i * rowsPerThread;
//			endRow = (i == NUM_THREADS - 1) ? NUM_Z_PLANE : (i + 1) * rowsPerThread;
//			threads.emplace_back(packSlicesRGBA, startRow, endRow, red, green, blue, numPoints, rgba3D.data());
//		}
//		for (auto& t : threads) { if (t.joinable()) t.join(); }
//		threads.clear();
//
...
//		compose.setVec2("uScreenSizePx", glm::vec2((float)NUM_DISPLAY_IMG_PX_X, (float)NUM_DISPLAY_IMG_PX_Y));
//		compose.setVec2("uTileSizePx", glm::vec2((float)NUM_ELEM_IMG_PX_X, (float)NUM_ELEM_IMG_PX_Y));
//		compose.setVec2("uNumLens", glm::vec2((float)NUM_LENS_X, (float)NUM_LENS_Y));
//
//		// invZ はフレーム毎に一度設定（UBOなし前提）
//		glUniform1fv(locInvZ, NZ, invZ.data());
//		glUniform1f(locInvZo, invZo);
//
//		// Bind textures
...
//
//	glFinish(); // 直前の描画完了を待つ
//	glPixelStorei(GL_PACK_ALIGNMENT, 1); // 念のため
//
//	//------------------------------
//
//
...
//	stream << "D:/ForStudy/reconstruction/PCSJ2025-OpenGL-" << VIEW_MODE
//		<< "-v1/PCSJ2025-OpenGL-" << VIEW_MODE
//		<< "-v1-mandrill_f" << std::fixed << std::setprecision(4) << (FOCAL_LENGTH * 1e3)
//		<< "_subsize" << std::fixed << std::setprecision(2) << (SUBJECT_SIZE_X * 1000.f)
//		<< "_zi" << (int)(SUBJECT_Z * 1000.f) << ".png";
...
//	//------------------------------
//
//	glDeleteTextures(1, &baseOffsetTex);

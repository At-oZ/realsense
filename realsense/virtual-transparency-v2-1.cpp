...
//#include <cmath>
//
//void insert_pixels(int Wpin, int Nz, int N, double D, double g, double wp, double pd, int starty, int endy, std::vector<double>& z_level, std::vector<std::vector<double>>& uo, std::vector<std::vector<double>>& vo, std::vector<std::vector<cv::Mat>>& elem_img, std::vector<std::vector<double>>& pointclouddata) {
//
//    g *= 0.001;
//    wp *= 0.001;
//    pd *= 0.001;
...
//    double inv_g = 1.0 / g, inv_wp = 1.0 / wp;
//    int nx, ny;
...
//
//        // 要素画像内の画素を埋める
//        insert_pixels(Wpin, Nz, N, D, g, wp, pd, 0, Hpin, std::ref(z_level), std::ref(uo), std::ref(vo), std::ref(elem_img), std::ref(pointclouddata));
//
//        // 要素画像内の画素を埋める(並列処理)
...
//        //    threads.emplace_back(insert_pixels, Hpin, Wpin, N, D, g, wp, startRow, endRow, std::ref(uo), std::ref(vo), std::ref(elem_img), std::ref(pointclouddata));
//        //}
//
//        //for (auto& t : threads) {
//        //    t.join();
...
//        stream << "v6_img_display_g" << g << "_wp" << std::fixed << std::setprecision(1) << wp << "_pd" << std::fixed << std::setprecision(3) << pd << "_D" << D << ".png"; // 小数点以下2桁で切り捨て

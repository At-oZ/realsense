...
//
//double max_z = 1000.0;
//int num_z_level = 50.0;
//double base = pow(max_z, 1.0 / num_z_level);
//
//// ファイル読み込み
...
//
//    double uo_ij, vo_ij, u, v, u0, v0, z, u_img, v_img, px_range, box_size, W_img, H_img;
...
//
//                        // 奥行きレベルにおける注目画素の位置
//                        u_img = (z + D) / g * (u - uo_ij) + uo_ij;
//                        v_img = (z + D) / g * (v - vo_ij) + vo_ij;
//
...
//                        if (-W_img / 2 <= u_img && u_img < W_img / 2 && -H_img / 2 <= v_img && v_img < H_img / 2) {
//
//                            // sum++;
//
//                            // イメージプレーン上の注目画素の画素番号
//                            x_img = floor((u_img + W_img / 2) / (W_img / WIDTH));
//                            y_img = floor((v_img + H_img / 2) / (H_img / HEIGHT));
//
...
//
//                            // 注目範囲内で点を探索
//                            for (int k = y_img - floor(box_size / 2); k <= y_img + floor(box_size / 2); k++) {
//                                for (int l = x_img - floor(box_size / 2); l <= x_img + floor(box_size / 2); l++) {
//
//                                    // cout << "k:" << k << ", l:" << l << endl;
//
//                                    // 点があったら
//                                    if (0 <= k && k < HEIGHT && 0 <= l && l < WIDTH) {
//                                        if (floor(pcd_box[k][l][zi] / (256 * 256 * 256)) == 1) {
//
//                                            // 点の色情報を要素画像の画素に埋め込む
//                                            // cout << "in!" << endl;
//                                            img_display[startv + m][startu + n] = pcd_box[k][l][zi] - 256 * 256 * 256;
...
//        stream << "mac_v10_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//

...
//int num_z_level = 250;
//int Ddash = 250;
//double coef = 1.0 * num_z_level * Ddash;
//
...
//    // bool flag2 = false;
//    int startu, startv;
//    double half_m, half_n, full_nx, full_ny, full_nz, tmp_nx, tmp_ny, tmp_uo, tmp_vo;
//
//    // 各要素画像の各画素ごとに
//    sum_zi = 0;
//    int sum_pcd_r = 0;
...
//
//        half_m = (double)m - N * 0.5;
//
//        for (int n = 0; n < N; n++) {
//
//            half_n = (double)n - N * 0.5;
//
//            // 各要素カメラごとに
...
//                    tmp_vo = vo[i * Wpin + j];
//
//                    // full_nx = (g / coef) * tmp_uo + (half_n + 0.5) * pd;
//                    // full_ny = (g / coef) * tmp_vo + (half_m + 0.5) * pd;
//
//                    tmp_nx = (g / coef) * tmp_uo * num_z_level + half_n * pd;
//                    tmp_ny = (g / coef) * tmp_vo * num_z_level + half_m * pd;
//
//                    // cout << "full nx:" << full_nx << ", full_ny:" << full_ny << endl;
//
//                    // 表示画像中における要素画像の左上画素の画素番号
...
//                        // full_nz = coef / (nz + 1);
//
//                        // full_nx = half_n * pd * full_nz / g + tmp_uo;
//                        // full_ny = half_m * pd * full_nz / g + tmp_vo;
//
//                        // tmp_nx = (g / coef) * tmp_uo * (nz + 1) + half_n * pd;
//                        // tmp_ny = (g / coef) * tmp_vo * (nz + 1) + half_m * pd;
//
//                        nx = static_cast<int>((tmp_nx + px_width_img * 0.5 * img_pitch) / img_pitch);
//                        ny = static_cast<int>((tmp_ny + px_height_img * 0.5 * img_pitch) / img_pitch);
//
...
//
//                            //            if (0 <= nx + l && nx + l < px_width_img && 0 <= ny + k && ny + k < px_height_img) {
//                            //                if (pcd_box[nz][ny + k][nx + l] > 0) {
//                            //                    tmp_pcd_r = static_cast<int>(floor((pcd_box[nz][ny + k][nx + l] - alpha) / (256 * 256)));
//                            //                    tmp_pcd_g = static_cast<int>(floor((pcd_box[nz][ny + k][nx + l] - alpha) / 256 - tmp_pcd_r * 256));
//                            //                    tmp_pcd_b = static_cast<int>(floor((pcd_box[nz][ny + k][nx + l] - alpha) - (tmp_pcd_r * 256 - tmp_pcd_g) * 256));
//
//                            //                    sum_pcd_r += tmp_pcd_r;
//                            //                    sum_pcd_g += tmp_pcd_g;
//                            //                    sum_pcd_b += tmp_pcd_b;
...
//                            //    }
//                            //    if (num_pcd > 0) {
//                            //        img_display[startv + m][startu + n] = static_cast<int>(round(sum_pcd_b / num_pcd)) + static_cast<int>(round(sum_pcd_g / num_pcd)) * 256 + static_cast<int>(round(sum_pcd_r / num_pcd)) * 256 * 256;
//                            //        break;
//                            //    }
//                            //}
//
//                        }
//
//                        tmp_nx -= (g / coef) * tmp_uo;
//                        tmp_ny -= (g / coef) * tmp_vo;
//
//                    }
//
//                }
...
//            inv_d = 1.0 / tmp_pcd_z;
//
//            newx = (g * inv_d) * tmp_pcd_x;
//            newy = (g * inv_d) * tmp_pcd_y;
//
//            nx = static_cast<int>((newx + px_width_img * 0.5 * img_pitch) / img_pitch);
//            ny = static_cast<int>((newy + px_height_img * 0.5 * img_pitch) / img_pitch);
...
//        stream << "./images/v11-4_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";
...
//
//        //    stream << "./images/imagePlane/v11-4_imagePlane_NumZLevel" << num_z_level << "_zi" << zi << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << ".png";
//        //    cout << "zi:" << zi << endl;
//

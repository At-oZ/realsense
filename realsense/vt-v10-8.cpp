...
///* 差分：各要素カメラに対してではなく、各イメージプレーンに対して繰り返す */
//
...
//}
//
//void insert_pixels(int start, int end, int** img_display, int*** pcd_box, int** x_img, int** y_img) {
//
//    int ximg, yimg, tmp_pcd_color, sum_zi, tmp_ximg, tmp_yimg;
//    int alpha = 256 * 256 * 256;
//    bool flag1 = false;
//    int px_wp;
//    double sum_wp_h, sum_wp_v, conv_wp;
//    int startu, startv;
//    // bool flag2 = false;
//
//    sum_wp_h = 0;
...
//
//                conv_wp = wp / (2 * zi * Htan) * px_width_img;
//
//                ximg = x_img[n][zi];
//                yimg = y_img[m][zi];
//
...
//                        if (0 <= tmp_ximg && tmp_ximg < px_width_img && 0 <= tmp_yimg && tmp_yimg < px_height_img) {
//
//                            // tmp_pcd_color = pcd_box[(yimg * WIDTH + ximg) * num_z_level + zi];
//                            // cout << "okkk" << endl;
//                            tmp_pcd_color = pcd_box[tmp_yimg][tmp_ximg][zi];
//                            // cout << "okk" << endl;
//                            
//                            if (tmp_pcd_color > 0) {
...
//
//                        sum_wp_h += conv_wp;
//                        tmp_ximg = ximg + round(sum_wp_h);
//                    }
//
//                    tmp_ximg = ximg;
//                    sum_wp_h = 0;
//                    sum_wp_v += conv_wp;
//                    tmp_yimg = yimg + round(sum_wp_v);
//                }                
//            }
//
//        }
...
//    for (int i = 0; i < N; i++) {
//        x_img[i] =(int*)malloc(sizeof(int) * num_z_level);
//        y_img[i] =(int*)malloc(sizeof(int) * num_z_level);
//    }
//
//    // int* x_img;
//    // int* y_img;
...
//
//                // イメージプレーン上の注目画素の画素番号
//                x_img[n][zi] = static_cast<int>(floor((u_img + W_img * 0.5) / (W_img / px_width_img)));
//                y_img[m][zi] = static_cast<int>(floor((v_img + H_img * 0.5) / (H_img / px_height_img)));
//
...
//                // if (x_img[n][zi] < 0 || px_width_img <= x_img[n][zi] || y_img[m][zi] < 0 || px_height_img <= y_img[m][zi]) {
//                //     x_img[n][zi] = -1;
//                //     y_img[m][zi] = -1;
//                // }
//
...
//        // cout << "ok" << endl;
//
//        //insert_pixels(0, N, std::ref(img_display), std::ref(pcd_box), std::ref(x_img), std::ref(y_img));
//
...
//             threads.emplace_back(insert_pixels, startRow, endRow, std::ref(img_display), std::ref(pcd_box), std::ref(x_img), std::ref(y_img));
//         }
//
//         for (auto& t : threads) {

...
int main(int argc, char* argv[]) {
    ...
    for (int idx_nph = 0; idx_nph < 1; idx_nph++) {
        ...
        for (int pt = 1; pt <= 5; pt++) {
            ...
            std::vector<std::vector<double>> array(46, std::vector<double>(78)); // 横：subz, 縦：nzl
            ...
            for (int nzl = 10; nzl <= 100; nzl += 2) {
                ...
                for (double subz = 300.0; subz <= 8000.0; subz += 100) {
                    ...
                    // 比較対象用の表示画像の読み込み
                    //std::string filename_display_compared = "C:/Users/taw11/EvacuatedStorage/prop-reconstruction/ICIP-DP/DP-mandrill_tileExpand_gridSize" +  std::to_string(nph) + "_zi" + std::to_string((int)subz) + ".png"; // 直接投影
                    ...
                    std::string filename_display_compared = "D:/EvacuatedStorage/prop-reconstruction/ICIP-prop-table-v2/prop-v2-detail-mandrill_tileExpand_pt" + std::to_string(pt) + "_Nz" + std::to_string(nzl) + "_subjectZ" + std::to_string((int)subz) + ".png"; // 提案手法（オリジナル版）
...
int writeCSV2(const std::vector<std::vector<double>> array, int ptimes) {
    ...
    // 出力ファイルを開く
    std::ofstream file("./numbers/PSNR/prop-table/psnr-real_prop-table-v2-pt" + std::to_string(ptimes) + "-mandrill_nph_tileExpand.csv");

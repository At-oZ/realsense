...
//
//    const double Df = -2.0; // 表示画像の設定に対するfocal lengthの差
//
//    // 画像を読み込む
...
//
//
//                        std::cout << "filename:" << filename_tile << ", NumPinhole:" << nph << ", NxNy:" << NxNy << ", NumZLevel:" << nzl << ", subjectZ:" << subz << ", pitchTimes:" << pt << std::endl;
//
//                        // ピンホールの設定
...
//                        const double tile_size = display_image_size; // 拡大の場合 * (subz + observer_z_min) / observer_z_minを付ける
//                        const double tile_px_size = tile_size / tile_px; // タイルに貼る画像のピクセルサイズ（mm）
//
//                        // 点群の設定
...
//
//                        // 比較対象用の表示画像の読み込み
//                        //std::string filename_display_compared = "D:/EvacuatedStorage/prop-reconstruction/ICIP-DP/DP-milkdrop_tileExpand_gridSize" +  std::to_string(nph) + "_zi" + std::to_string((int)subz) + ".png"; // 直接投影
...
//                        //std::string filename_display_compared = "D:/EvacuatedStorage/prop-reconstruction/ICIP-prop-table-v1/prop-table-v1-milkdrop_tileExpand_Nz" + std::to_string(nzl) + "_subjectZ" + std::to_string((int)subz) + ".png"; // 提案手法（テーブル版v1）
//                        //std::string filename_display_compared = "D:/EvacuatedStorage/prop-reconstruction/ICIP-prop-table-v1/prop-v2-milkdrop_tileExpand_pt" + std::to_string(pt) + "_Nz" + std::to_string(nzl) + "_subjectZ" + std::to_string((int)subz) + ".png"; // 提案手法（テーブル版v2）
//                        //std::string filename_display_compared = "C:/Users/taw11/EvacuatedStorage/prop-reconstruction/ICIP-prop-improve-v1/prop-improve-v1-milkdrop_tileExpand_Nz" + std::to_string(nzl) + "_N" + std::to_string((int)ptimes) + "_subjectZ" + std::to_string((int)subz) + ".png"; //  提案手法(完成版1)
//                        //std::string filename_display_compared = "D:/EvacuatedStorage/prop-reconstruction/ICIP-prop-improve-v1/prop-improve-v1-detail-milkdrop_tileExpand_Nz" + std::to_string(nzl) + "_N" + std::to_string((int)ptimes) + "_subjectZ" + std::to_string((int)subz) + ".png"; //  提案手法(完成版2)
//                        std::string filename_display_compared = "E:/EvacuatedStorage/prop-reconstruction/prop-improve-v1/prop-improve-v1-grid1_tileNotExpand_Nx" + std::to_string(NxNy) + "_Ny" + std::to_string(NxNy) + "_Nz" + std::to_string(nzl) + "_N" + std::to_string(pt) + "_zi" + std::to_string((int)subz) + ".png";  //  提案手法(完成版3)
...
//
//                                // 比較対象の観察画像
//                                cv::Mat compared_image = ideal_observe(display_compared_image, nph, pinhole_pitch, focal_length + Df, observer_x[nobs], observer_y[nobs], observer_z, pinhole_size, display_image_size, display_px_size, display_image_px);
...
//
//    // 出力ファイルを開く
//    std::ofstream file("./numbers/PSNR/prop-improve/by_real/detail/psnr-milkdrop_real_prop-improve-NxNy" + std::to_string(NxNy) + "_pt" + std::to_string(ptimes) + "-nph_tileExpand.csv");
//
//    // ファイルが正しく開けたか確認
//    if (!file.is_open()) {
//        std::cerr << "ファイルが開けませんでした" << std::endl;

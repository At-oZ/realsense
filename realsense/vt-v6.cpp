...
///* 点群データのx, y座標を左上の要素カメラを基準にしたものに変換し, 各要素画像の各画素には視差の分だけずらしながら埋める */
//
...
//        }
//
//        // insert_pixels(0, WIDTH * HEIGHT, std::ref(img_display), std::ref(data), std::ref(val_z));
//
...
//            endRow = (i == numThreads - 1) ? (WIDTH * HEIGHT) : (i + 1) * rowsPerThread;
...
//        stream << "./images/v6_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//

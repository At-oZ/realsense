...
///* v5との差分：基準カメラを縦横2x2の4箇所に配置し、それをもとに視差を用いて要素画像群を埋める */
//
...
//}
//
//void insert_pixels(int start, int end, int* img, int** img_display, double** val_z, double* uo, double* vo) {
//
//    int nx, ny;
//    int startv, startu;
//    int ref_idx_x, ref_idx_y;
//    double pos_cam_x, pos_cam_y;
//
...
//
//            ref_idx_x = static_cast<int>(floor(j * num_ref_view / Wpin));
//            ref_idx_y = static_cast<int>(floor(i * num_ref_view / Hpin));
//
//
//            // cout << "ref_idx_x: " << ref_idx_x << ", ref_idx_y: " << ref_idx_y << endl;
//
//            pos_cam_x = wp * ((j + 0.5) - Wpin * 0.5);
//            pos_cam_y = wp * ((i + 0.5) - Hpin * 0.5);
//
//            // cout << "pos cam x: " << pos_cam_x << ", pos cam y: " << pos_cam_y << endl;
//
...
//
//                    nx = l - static_cast<int>(floor((pos_cam_x - uo[ref_idx_y * num_ref_view + ref_idx_x]) * (g / (pd * val_z[ref_idx_y * (N * range) + k][ref_idx_x * (N * range) + l] * 1000))));
//                    ny = k - static_cast<int>(floor((pos_cam_y - vo[ref_idx_y * num_ref_view + ref_idx_x]) * (g / (pd * val_z[ref_idx_y * (N * range) + k][ref_idx_x * (N * range) + l] * 1000))));
//
//                    // cout << (pos_cam_x - uo[ref_idx_y * num_ref_view + ref_idx_x]) * (g / (pd * val_z[ref_idx_y * (N * range) + k][ref_idx_x * (N * range) + l] * 1000)) << endl;
//
//                    // cout << "l:" << l << ", k:" << k << endl;
...
//                            img_display[startv + ny][startu + nx] = img[((ref_idx_y * num_ref_view + ref_idx_x) * (N * range) + k) * (N * range) + l];
//                    }
//                }
//            }
//
...
//
//            uo[i * num_ref_view + j] = (Wpin * wp) * ((j + 0.5) / num_ref_view - 0.5);
//            vo[i * num_ref_view + j] = (Hpin * wp) * ((i + 0.5) / num_ref_view - 0.5);
//
//            // cout << "uo:" << uo[i * num_ref_view + j] << ", vo:" << vo[i * num_ref_view + j] << endl;
//
//        }
//    }
//
...
//
//    double** val_z;
//    val_z = (double**)malloc(sizeof(double*) * N * range * num_ref_view);
//    for (int i = 0; i < N * range * num_ref_view; i++) {
//        val_z[i] = (double*)malloc(sizeof(double) * N * range * num_ref_view);
//    }
//
//    int WIDTH = 640;
//    int HEIGHT = 480;
...
//    int* img;
//    img = (int*)malloc(sizeof(int) * ((N * range) * num_ref_view) * ((N * range) * num_ref_view));
//
//    // フレーム処理
//    for (int tt = 0; tt < 1; tt++) {
//
//        for (int i = 0; i < N * range * num_ref_view; i++) {
//            for (int j = 0; j < N * range * num_ref_view; j++) {
//                val_z[i][j] = 10;
//            }
//        }
//
...
//
//                    if (0 <= nx && nx < N * range && 0 <= ny && ny < N * range) {
//                        if (val_z[i * (N * range) + ny][j * (N * range) + nx] > tmp_pcd2) {
//
...
//
//                            img[((i * num_ref_view + j) * (N * range) + ny) * (N * range) + nx] = static_cast<unsigned char>(tmp_pcd3) + static_cast<unsigned char>(tmp_pcd4) * 256 + static_cast<unsigned char>(tmp_pcd5) * 256 * 256;
//
//                            val_z[i * (N * range) + ny][j * (N * range) + nx] = tmp_pcd2;
//                        }
//                    }
//                }
//            }
...
//
//        // insert_pixels(0, Hpin, std::ref(img), std::ref(img_display), std::ref(val_z), std::ref(uo), std::ref(vo));
//
//        int startRow, endRow;
//        for (int i = 0; i < numThreads; i++) {
//            startRow = i * rowsPerThread;
//            endRow = (i == numThreads - 1) ? Hpin : (i + 1) * rowsPerThread;
//            threads.emplace_back(insert_pixels, startRow, endRow, std::ref(img), std::ref(img_display), std::ref(val_z), std::ref(uo), std::ref(vo));
//        }
//
//        for (auto& t : threads) {
...
//        // for (int i = 0; i < num_ref_view; i++) {
//        //     for (int j = 0; j < num_ref_view; j++) {
//        //         for (int ny = 0; ny < (N * range); ny++) {
//        //             for (int nx = 0; nx < (N * range); nx++) {
//        //                 img_save.at<cv::Vec3b>(i * (N * range) + ny, j * (N * range) + nx)[2] = static_cast<int>(floor(img[((i * num_ref_view + j) * (N * range) + ny) * (N * range) + nx] / (256 * 256)));
//        //                 img_save.at<cv::Vec3b>(i * (N * range) + ny, j * (N * range) + nx)[1] = static_cast<int>(floor(img[((i * num_ref_view + j) * (N * range) + ny) * (N * range) + nx] / 256 - img_save.at<cv::Vec3b>(i, j)[2] * 256));
//        //                 img_save.at<cv::Vec3b>(i * (N * range) + ny, j * (N * range) + nx)[0] = static_cast<int>(floor(img[((i * num_ref_view + j) * (N * range) + ny) * (N * range) + nx] - (img_save.at<cv::Vec3b>(i, j)[2] * 256 - img_save.at<cv::Vec3b>(i, j)[1]) * 256));
//        //             }
//        //         }
//        //     }
//        // }
//
//        ostringstream stream;
//        // stream << "mac_v5-2_img_ref_view" << num_ref_view << "_range" << range << "_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//        stream << "mac_v5-2_img_display_view" << num_ref_view << "_range" << range << "_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//

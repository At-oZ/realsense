...
//                    double subject_size = pinhole_array_size * (subz + zo_min) / zo_min; // 被写体のサイズ(拡大する場合 * (subz + zo_min) / zo_minを追加);
...
//                    int TIMES = 1;
//
//                    cout << "NumPinhole:" << num_pinhole << ", ptimes:" << pt << ", NumZLevel:" << num_z_level << ", subjectZ:" << subject_z << endl;
//
//                    // 各要素画像の原点画素位置(左上)
...
//
//    // 出力ファイルを開く
//    std::ofstream file("./numbers/speed/speed-prop-table-v2-pt" + std::to_string(ptimes) + "-mandrill-tileExpand.csv");
//
//    // ファイルが正しく開けたか確認
//    if (!file.is_open()) {

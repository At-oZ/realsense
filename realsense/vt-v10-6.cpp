...
///* 差分：x_img, y_img(投影線がイメージプレーンに当たる位置)を先に求めて配列として保存する */
//
...
//                        if (0 <= ximg && ximg < WIDTH && 0 <= yimg && yimg < HEIGHT) {
//
...
//
//                        // イメージプレーン上の注目画素の画素番号
//                        x_img[n][j][zi] = static_cast<int>(floor((u_img + W_img * 0.5) / (W_img * inv_WIDTH)));
//                        y_img[m][i][zi] = static_cast<int>(floor((v_img + H_img * 0.5) / (H_img * inv_HEIGHT)));
//
//                        // x_img[(n * Wpin + j) * num_z_level + zi] = static_cast<int>(floor((u_img + W_img * 0.5) / (W_img * inv_WIDTH)));
//                        // y_img[(m * Hpin + i) * num_z_level + zi] = static_cast<int>(floor((v_img + H_img * 0.5) / (H_img * inv_HEIGHT)));
//
//                    }
//
//                    uo += wp;
...
//                H_img = 2 * z * Vtan;
//
//                ximg = floor((tmp_pcd_x * 1000 + W_img * 0.5) / (W_img / WIDTH));
//                yimg = floor((tmp_pcd_y * 1000 + H_img * 0.5) / (H_img / HEIGHT));
//
//                if (0 <= ximg && ximg < WIDTH && 0 <= yimg && yimg < HEIGHT) {
//
//                    // cout << "in!" << endl;
//                    // pcd_box[(yimg * WIDTH + ximg) * num_z_level + z_level] = static_cast<unsigned char>(data[k][3]) + static_cast<unsigned char>(data[k][4]) * 256 + static_cast<unsigned char>(data[k][5]) * 256 * 256 + 256 * 256 * 256;
...
//        stream << "mac_v10-6_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//

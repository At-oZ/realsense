...
///* 差分:計算の高速化の検討(v1-4の変更点 & 計算のメモリアクセス化(xt, ytのみ)) */
//
...
//}
//
//void insert_pixels(int start, int end, cv::Mat img_display, unsigned char*** red, unsigned char*** green, unsigned char*** blue, bool*** alpha, double* sp, double* tp, double** xt, double** yt) {
//
...
//
//                        nx = static_cast<int>(floor((g / img_pitch) * tmp_xt + 0.5) + px_width_img * 0.5);
//                        ny = static_cast<int>(floor((g / img_pitch) * tmp_yt + 0.5) + px_height_img * 0.5);
//
...
//
//                        //zt -= inv_coef;
//                        tmp_xt -= tmp_sp * inv_coef;
//                        tmp_yt -= tmp_tp * inv_coef;
//                    }
//
//
//                }
...
//            nx = static_cast<int>(floor((g / img_pitch) * tmp_xt + 0.5) + px_width_img * 0.5);
//            ny = static_cast<int>(floor((g / img_pitch) * tmp_yt + 0.5) + px_height_img * 0.5);
//            nz = static_cast<int>(floor(coef * tmp_zt + 0.5));
//
...
//            threads.emplace_back(insert_pixels, startRow, endRow, std::ref(img_display), std::ref(red), std::ref(green), std::ref(blue), std::ref(alpha), std::ref(sp), std::ref(tp), std::ref(xt), std::ref(yt));
//        }
//
//        for (auto& t : threads) {
...
//        stream << "./images/prop-v1-6_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";

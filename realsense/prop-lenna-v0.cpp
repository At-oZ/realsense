...
//}
//
//void insert_pixels(int start, int end, int element_image_px, double display_pixel_pitch, int num_pinhole, double intv, int num_z_level, double inv_coef, double gap, double img_pitch, int px_width_img, int px_height_img, cv::Mat img_display, int*** red, int*** green, int*** blue, bool*** alpha, double* sp, double* tp) {
//
...
//
//                        nx = static_cast<int>(floor((gap / img_pitch) * xt + 0.5) + px_width_img * 0.5);
//                        ny = static_cast<int>(floor((gap / img_pitch) * yt + 0.5) + px_height_img * 0.5);
//
...
//
//                int px_height_img = static_cast<int>(round(55));
//                int px_width_img = static_cast<int>(round(55));
//
//                int TIMES = 100;
//
...
//                        threads.emplace_back(insert_pixels, startRow, endRow, element_image_px, display_pixel_pitch, num_pinhole, intv, num_z_level, inv_coef, gap, img_pitch, px_width_img, px_height_img, img_display, red, green, blue, alpha, sp, tp);
//                    }
//
//                    for (auto& t : threads) {
...
//                // 表示画像の保存
//                ostringstream stream;
//                stream << "./images/lenna/prop-reconstruction/v0/comparison/prop-lenna-v0_ImgDisplay_NumPinhole" << num_pinhole << "_NumZLevel" << num_z_level << "_subjectZ" << (int)subject_z << ".png";
//                cv::String filename = stream.str();
//                imwrite(filename, img_display);
//
//

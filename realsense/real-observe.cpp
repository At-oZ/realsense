...
//        for (int j = 0; j < Wpin; ++j) {
//            elem_img[i][j] = cv::Mat(cv::Size(Nreal, Nreal), CV_8UC3, cv::Scalar(0, 0, 0));
//        }
//    }
//
//    // 切り抜いた要素画像群
//    std::vector<std::vector<cv::Mat>> cliped_elem_img(Hpin, std::vector<cv::Mat>(Wpin));
//    for (int i = 0; i < Hpin; ++i) {
//        for (int j = 0; j < Wpin; ++j) {
//            cliped_elem_img[i][j] = cv::Mat(cv::Size(N, N), CV_8UC3, cv::Scalar(0, 0, 0));
//        }
//    }
//
//    // 視点パラメータ(mm)
...
//        for (int j = 0; j < Wpin; j++) {
//
//            uo[i][j] = (j - static_cast<int>(floor((Wpin - 1) * 0.5))) * wp;
//            vo[i][j] = (i - static_cast<int>(floor((Hpin - 1) * 0.5))) * wp;
//
...
//            threads.emplace_back(insert_pixels, Hpin, Wpin, Nreal, D, zo, W, startRow, endRow, std::ref(uo), std::ref(vo), std::ref(elem_img), std::ref(pointclouddata));
//        }
//
//        for (auto& t : threads) {
//            t.join();
//        }
//
//        std::cout << "inserting finished" << std::endl;
//
//        int startu, startv, endu, endv;
//        double inv2W = 1.0 / (2.0 * W);
//        for (int i = 0; i < Hpin; i++) {
//            for (int j = 0; j < Wpin; j++) {
//                
//                std::cout << "i:" << i << ", j:" << j << std::endl;
//
//                startu = static_cast<int>(floor((W * 0.5 - uo[i][j]) * inv2W * Nreal));
//                startv = static_cast<int>(floor((W * 0.5 - vo[i][j]) * inv2W * Nreal));
//
//                //endu = static_cast<int>(floor((W * 1.5 - uo[i][j]) * inv2W * Nreal));
//                //endv = static_cast<int>(floor((W * 1.5 - vo[i][j]) * inv2W * Nreal));
//
//                //std::cout << "num u:" << endu - startu << ", num v:" << endv - startv << std::endl;
//
//                for (int v = 0; v < N; v++) {
//                    for (int u = 0; u < N; u++) {
//

...
///* prop-lenna-v1-2からの派生 */
///*  テーブル版 (最近傍処理)(詳細化と平均化なし) */
//
...
//
//                int px_height_img = static_cast<int>(round(50 * focal_length / 4.0));
//                int px_width_img = static_cast<int>(round(50 * focal_length / 4.0));
//
//                int TIMES = 1;
//
...
//                // 表示画像の保存
//                ostringstream stream;
//                stream << "C:/Users/taw11/EvacuatedStorage/prop-reconstruction/ICIP-prop-table-v1/prop-table-v1-milkdrop_subjectZ" << (int)subz << ".png";
//                cv::String filename = stream.str();
//                imwrite(filename, img_display);
//
//
//                cout << "平均実行時間: " << sum_time / TIMES << " ms" << std::endl;
//                array[0][idx_subz++] = sum_time / TIMES;
//
//                // 使用したメモリを解放
//                for (int i = 0; i < rows; ++i) {
//                    free(data[i]);
...
//
//    // 出力ファイルを開く
//    std::ofstream file("C:/Users/taw11/EvacuatedStorage/prop-reconstruction/speed-prop-table-v1-milkdrop.csv");
//
//    // ファイルが正しく開けたか確認
//    if (!file.is_open()) {

...
//    bool flag1 = false;
//    // bool flag2 = false;
//    int startu, startv, u, v;
//    double axt, ayt, azt, xp, yp, zp, newu, newv;
//
...
//
//                    //azt = ((double)nz - 0.5) / coef;
//                    //axt = ((double)nx - 0.5 - (double)px_width_img * 0.5) * (img_pitch / g);
//                    //ayt = ((double)ny - 0.5 - (double)px_height_img * 0.5) * (img_pitch / g);
//
//                    ////cout << "diff xt:" << xt - axt << ", diff yt:" << yt - ayt << ", diff zt:" << zt - azt << endl;
//
//                    //zp = 1.0 / azt;
//                    //xp = axt / azt;
//                    //yp = ayt / azt;
//
//                    zp = coef / ((double)nz - 0.5);
//                    xp = zp * ((double)nx - 0.5 - (double)px_width_img * 0.5) * (img_pitch / g);
//                    yp = zp * ((double)ny - 0.5 - (double)px_height_img * 0.5) * (img_pitch / g);
//
//                    for (int i = start; i < end; i++) {
//                        for (int j = 0; j < Wpin; j++) {
//                            
//                            newu = (g / zp) * (xp - sp[i * Wpin + j]);
//                            newv = (g / zp) * (yp - tp[i * Wpin + j]);
//
//                            u = static_cast<int>(floor(newu / pd) + N * 0.5);
//                            v = static_cast<int>(floor(newv / pd) + N * 0.5);
//
...
//
//    bool** flag_z;
//    flag_z = (bool**)malloc(sizeof(bool*) * ph_disp);
//    for (int i = 0; i < ph_disp; i++) {
//        flag_z[i] = (bool*)malloc(sizeof(bool) * pw_disp);
//    }
//
//    // // 箱の全引き出しに白を格納（テスト用）
//    // for (int i = 0; i < num_z_level; i++) {
...
//        double xt, yt, zt, axt, ayt, azt, xp, yp, zp;
//        int nx, ny, nz;
//        int pcd_count = 0;
//
//         //点群を箱に格納
...
//                //yp = ayt / azt;
//
//                //cout << "diff x:" << tmp_pcd_x - xp << ", diff y:" << tmp_pcd_y - yp << ", diff z:" << tmp_pcd_z - zp << endl;
//

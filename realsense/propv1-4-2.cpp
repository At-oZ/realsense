...
///* 差分:計算の高速化の検討(v1-4からimg_displayをcvMatから配列に変更) */
//
...
//}
//
//void insert_pixels(int start, int end, int** img_display, unsigned char*** red, unsigned char*** green, unsigned char*** blue, bool*** alpha, double* sp, double* tp) {
//
...
//
//                                img_display[startv + m][startu + n] = static_cast<unsigned char>(blue[nz][ny][nx]) + static_cast<unsigned char>(green[nz][ny][nx]) * 256 + static_cast<unsigned char>(red[nz][ny][nx]) * 256 * 256;
//
...
//        stream << "./images/prop-v1-4-2_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";

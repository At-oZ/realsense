...
///* v4-2との差分：X Window Systemを用いて出力 */
//
...
//}
//
//void insert_pixels(int starty, int endy, unsigned char* img, unsigned char* img_display, double** val_z) {
//
//    int nx, ny;
//    int startv, startu;
//
...
//                            img_display[((startv + ny) * pw_disp + (startu + nx)) * 3] = img[(l * (N * range) + k) * 3];
//                            img_display[((startv + ny) * pw_disp + (startu + nx)) * 3 + 1] = img[(l * (N * range) + k) * 3 + 1];
//                            img_display[((startv + ny) * pw_disp + (startu + nx)) * 3 + 2] = img[(l * (N * range) + k) * 3 + 2];
//                    }
//                }
//            }
//
...
//    unsigned char* img_display;
//    img_display = (unsigned char*)malloc(sizeof(unsigned char) * pw_disp * ph_disp * 3);
//    unsigned char* img;
//    img = (unsigned char*)malloc(sizeof(unsigned char) * (N * range) * (N * range) * 3);
//
//    // フレーム処理
...
//
//                    img[(ny * (N * range) + nx) * 3] = static_cast<unsigned char>(tmp_pcd3);
//                    img[(ny * (N * range) + nx) * 3 + 1] = static_cast<unsigned char>(tmp_pcd4);
//                    img[(ny * (N * range) + nx) * 3 + 2] = static_cast<unsigned char>(tmp_pcd5);
//
...
//                XPutPixel(image, x, y, img_display[(y * pw_disp + x) * 3] + img_display[(y * pw_disp + x) * 3 + 1] * 256 + img_display[(y * pw_disp + x) * 3 + 2] * 256 * 256);
//            }
//        }
//
//        // メインループ

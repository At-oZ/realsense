...
//
//        double zo_min = 1500.0; // 最短視認距離(mm)
//        // ピクセルサイズの計算
...
//
//        // カメラの設定
//        const double focalLength = zo_min / (2.0 / (1.0 / (double)nph - 0.15 / (double)(displayImageSize * pixelSize)) - 1.0); // 焦点距離（mm）
...
//
//            // 画像の保存
//            std::string filename = "./images/lenna/prop-reconstruction/ideal_pixelSize" + std::to_string(static_cast<int>(pixelSize * 1000)) + "um/ideal_captured_image_imageResolution" + std::to_string(static_cast<int>(imageResolution)) + "_gridSize" + std::to_string(static_cast<int>(gridSize)) + "_zi" + std::to_string(static_cast<int>(tileZ)) + ".png";
//

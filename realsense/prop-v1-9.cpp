...
///* 差分:計算の高速化の検討(v1-8の変更点 & 点群の集約を並列化)→ 70ms から 68ms の短縮, 17.2927 dB から 平均16.68 dBに減少... */
//
...
//}
//
//void aggregate_pointcloud(int start, int end, double** data, int*** red, int*** green, int*** blue, bool*** alpha, int*** num_pcd) {
//
...
//            //if (pcd_box[tmp_nz][tmp_ny][tmp_nx] > 0) {
//            //    pcd_count++;
//            //}
//
...
//            //}
//            // cout << "pcd_box:" << pcd_box[tmp_nz][tmp_ny][tmp_nx] << endl;
//        }
//    }
//
//    // スレッドの終了をカウントし、条件変数を通知する
...
//            threads.emplace_back(aggregate_pointcloud, startRow, endRow, std::ref(data), std::ref(red), std::ref(green), std::ref(blue), std::ref(alpha), std::ref(num_pcd));
//        }
//
//        for (auto& t : threads) {
...
//        stream << "./images/prop-v1-9_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";
...
//        stream.clear(ostringstream::goodbit);
//
//        stream << "C:/Users/taw11/source/repos2/At-oZ/realsense/realsense/images/prop-v1-9_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";
//

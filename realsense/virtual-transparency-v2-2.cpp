...
//#include <cmath>
//
//void insert_pixels(int Wpin, int N, double D, double g, double wp, double pd, int starty, int endy, std::vector<std::vector<double>>& uo, std::vector<std::vector<double>>& vo, std::vector<std::vector<cv::Mat>>& elem_img, std::vector<std::vector<double>>& pointclouddata) {
//
//    g *= 0.001;
//    wp *= 0.001;
//    pd *= 0.001;
...
//    double inv_g = 1.0 / g, inv_wp = 1.0 / wp, inv_pd = 1.0 / pd;
//    double x, y;
//    //int nx, ny;
...
//
//        // 要素画像内の画素を埋める
//        insert_pixels(Wpin, N, D, g, wp, pd, 0, Hpin, std::ref(uo), std::ref(vo), std::ref(elem_img), std::ref(pointclouddata));
//
//        // //要素画像内の画素を埋める(並列処理)

...
//    int i, j, k, l;
//
//    for (k = 0; k < pointclouddata.size(); k++) {
//
...
//
//        for (l = 0; l < (endy - starty) * Wpin; l++) {
//
//            i = static_cast<int>(floor(l / Wpin));
//            j = l % Wpin;
//
...
//        }
//
//        //for (k = pw_disp - 1; k < (ph_disp - 1) * (pw_disp - 1); k++) {
//
//        //    j = k % (pw_disp - 1);
//
//        //    if (j > 0) {
//
//        //        i = static_cast<int>(floor(k / (pw_disp - 1)));
//
//        //        //std::cout << "i:" << i << ", j:" << j << std::endl;
//
...
//        stream << "v_7_img_display_g" << g << "_wp" << std::fixed << std::setprecision(1) << wp << "_pd" << std::fixed << std::setprecision(3) << pd << "_D" << D << ".png"; // 小数点以下2桁で切り捨て

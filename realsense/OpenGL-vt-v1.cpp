// OpenGL-vt-v1.cpp
// Stage1: 40×40 並行カメラ (z=0, +Z 方向) タイル描画
// Stage2: 概観カメラ (位置固定 0,0,-1 + 動的FOV)
// 追加: Stage1 のタイル全体 (WIN_W×WIN_H) を 1 枚の PNG として初回フレームのみ保存 (stage1_full.png)
...
layout(location=1) in vec3 inCol;   // 現在: B,G,R の順で入ってくる
uniform mat4 uMVP;
out vec3 vCol;
void main(){
...
int main() {
    ...
    const float subject_size = pinhole_array_width;
    ...
    while (!glfwWindowShouldClose(gridWin)) {
        ...
        for (int r = 0; r < CAM_ROWS; ++r) {
            for (int c = 0; c < CAM_COLS; ++c) {
                ...
                int vx = c * VIEW_W;
                int vy = (CAM_ROWS - 1 - r) * VIEW_H; // 上から下へ
        ...
        if (!savedFull) {
            ...
            std::string outPath = "output/stage1_full.png";

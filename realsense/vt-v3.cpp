...
///* v2との差分：openmpではなくthreadで実装. 補間を使用 */
//
...
//                //cout << "i:" << i << ", j:" << j << endl;
//
//                if(i % interp_intv == 0 && j % interp_intv == 0){
...
//}
//
//void interpole_horizontal(int Hpin, int N, int startx, int endx, double intv, cv::Mat img_display){
//
//    // cout << "interpole horizontal start" << endl;
//
//    int startv, startu, stdu;
//
//    for (int i = 0; i < Hpin; i += interp_intv) {
...
//                startu = static_cast<int>(j * intv);
//
//                stdu = static_cast<int>(floor(j / interp_intv) * interp_intv * intv);
//
//                for (int u = 0; u < N; u++) {
//                    for (int v = 0; v < N; v++) {
//
//                        // cout << "num of horizontal shift px: " << j * (wp * g) / (pd * val_z[((i * Wpin) * N + v) * N + u] * 1000) << endl;
//                        // cout << "num of vertical shift px: " << i * (wp * g) / (pd * val_z[(j * N + v) * N + u] * 1000)<< endl;
//
//                        img_display.at<cv::Vec3b>(startv + v, startu + u)[0] = img_display.at<cv::Vec3b>(startv + v, stdu + u)[0];
//                        img_display.at<cv::Vec3b>(startv + v, startu + u)[1] = img_display.at<cv::Vec3b>(startv + v, stdu + u)[1];
//                        img_display.at<cv::Vec3b>(startv + v, startu + u)[2] = img_display.at<cv::Vec3b>(startv + v, stdu + u)[2];
//                    }
//                }
//            }
//        }
...
//}
//
//void interpole_vertical(int Hpin, int N, int startx, int endx, double intv, cv::Mat img_display){
//
//    // cout << "interpole vertical start" << endl;
//
//    int startv, startu, stdv;
//
//    for (int j = startx; j < endx; j++) {
...
//                startu = static_cast<int>(j * intv);
//
//                stdv = static_cast<int>(floor(i / interp_intv) * interp_intv * intv);
//
//                for (int u = 0; u < N; u++) {
//                    for (int v = 0; v < N; v++) {
//
...
//                        // cout << "num of vertical shift px: " << i * (wp * g) / (pd * val_z[(j * N + v) * N + u] * 1000)<< endl;
//
//                        img_display.at<cv::Vec3b>(startv + v, startu + u)[0] = img_display.at<cv::Vec3b>(stdv + v, startu + u)[0];
//                        img_display.at<cv::Vec3b>(startv + v, startu + u)[1] = img_display.at<cv::Vec3b>(stdv + v, startu + u)[1];
//                        img_display.at<cv::Vec3b>(startv + v, startu + u)[2] = img_display.at<cv::Vec3b>(stdv + v, startu + u)[2];
//                    }
//                }
//            }
//        }
...
//            startRow = i * rowsPerThread;
//            endRow = (i == numThreads - 1) ? Hpin : (i + 1) * rowsPerThread;
//            threads.emplace_back(interpole_horizontal, Hpin, N, startRow, endRow, intv, img_display);
//        }
//
//        for (auto& t : threads) {
//            if(t.joinable()){t.join();}
...
//        }
//
//        cout << "interpole horizontal finished" << endl;
//        finished_threads = 0;
//
//        for (int i = 0; i < numThreads; i++) {
//            startRow = i * rowsPerThread;
//            endRow = (i == numThreads - 1) ? Hpin : (i + 1) * rowsPerThread;
//            threads.emplace_back(interpole_vertical, Hpin, N, startRow, endRow, intv, img_display);
//        }
//
//        for (auto& t : threads) {
//            if(t.joinable()){t.join();}
//        }
//
//        cout << "interpole vertical finished" << endl;
//
//        //int startv, startu;
//        //for (int i = 0; i < Hpin; i++) {
//        //    for (int j = 0; j < Wpin; j++) {
...
//        stream << "mac_v3_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";

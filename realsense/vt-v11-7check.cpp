...
//
//// パラメータ
//int num_z_level = 2400; // イメージプレーンの層数
//double ptimes = 1; // イメージプレーンの画素ピッチの係数(1でディスプレイの画素ピッチと同じ)
//int box_size = 1; // 探索範囲
//
//double min_d = 250.0; // ディスプレイと一番手前のイメージプレーンの距離(+D mm)
//double max_d = 2650.0; // ディスプレイと一番奥のイメージプレーンの距離(+D mm)
//
...
//}
//
//void insert_pixels(int start, int end, int** img_display, int*** pcd_box, double* sp, double* tp, double** data) {
//
//    int nu, nv, tmp_pcd_color, sum_zi, tmp_pcd_b, tmp_pcd_g, tmp_pcd_r;
//    int alpha = 256 * 256 * 256;
//    bool flag1 = false;
//    // bool flag2 = false;
//    int startu, startv;
//    double u, v, xt, yt, zt, diff_xt, diff_yt, diff_zt;
//    double min_diff;
//    int kk;
//
//    for (int nz = num_z_level - 1; nz >= 0; nz--) {
//
//        zt = (double)nz / (double)(num_z_level - 1) * (1.0 / min_d - 1.0 / max_d) + 1.0 / max_d;
//
//        for (int ny = 0; ny < px_height_img; ny++) {
//
//            //yt = img_pitch / g * ((ny + 0.5) - px_height_img * 0.5);
//            yt = img_pitch / (2 * g) * (2.0 * ((double)ny - px_height_img / 2.0) + 1.0);
//
//            for (int nx = 0; nx < px_width_img; nx++) {
//
//                //xt = img_pitch / g * ((nx + 0.5) - px_width_img * 0.5);
//                xt = img_pitch / (2 * g) * (2.0 * ((double)nx - px_height_img / 2.0) + 1.0);
//
...
//                    //    diff_xt = xt - data[k][0] * 1000.0 / ((double)tt + D);
//                    //    diff_yt = yt - data[k][1] * 1000.0 / ((double)tt + D);
//                    //    diff_zt = zt - (1.0 / ((double)tt + D));
//
//                    //    //cout << "k:" << k << ", diff xt:" << diff_xt << ", diff yt:" << diff_yt << ", diff zt:" << diff_zt << endl;
//
//                    //    if (min_diff > (diff_xt * diff_xt + diff_yt * diff_yt)) {
//                    //        kk = k;
//                    //        min_diff = diff_xt * diff_xt + diff_yt * diff_yt;
...
//                    //cout << "kk:" << kk << endl;
//
//                    //xt = img_pitch / g * ((nx + beta[kk][0]) - px_width_img * 0.5);
//                    //yt = img_pitch / g * ((ny + beta[kk][1]) - px_height_img * 0.5);
//
...
//                            //cout << "i:" << i << ", j:" << j << endl;
//                            
//                            //xt = data[kk][0] * 1000.0 / ((double)tt + D);
//                            //yt = data[kk][1] * 1000.0 / ((double)tt + D);
//                            //zt = 1.0 / ((double)tt + D);
//
//                            u = g * (xt - zt * sp[i * Wpin + j]);
//                            v = g * (yt - zt * tp[i * Wpin + j]);
//
//                            nu = static_cast<int>(floor(u / pd) + N * 0.5);
//                            nv = static_cast<int>(floor(v / pd) + N * 0.5);
//
...
//                                if (img_display[startv + nv][startu + nu] == 0) {
//
//                                    img_display[startv + nv][startu + nu] = pcd_box[nz][ny][nx] - alpha;
//                                }
//                            }
//                        }
//                    }
...
//    //    //v = wp * 0.5 * ((double)(2 * m + 1) / (double)N - 1.0);
//
//    //    for (int n = 0; n < N; n++) {
//
//    //        u = ((double)n - (double)(N - 1) * 0.5) * pd;
//    //        //u = wp * 0.5 * ((double)(2 * n + 1) / (double)N - 1.0);
//
//    //        // 各要素カメラごとに
...
//
//    double** beta;
//    beta = (double**)malloc(sizeof(double*) * rows);
//    for (int i = 0; i < rows; i++) {
//        beta[i] = (double*)malloc(sizeof(double) * 2);
//    }
//
//    // // 箱の全引き出しに白を格納（テスト用）
//    // for (int i = 0; i < num_z_level; i++) {
...
//            nx = static_cast<int>(floor((g / img_pitch) * xt + 0.5) + px_width_img * 0.5);
//            ny = static_cast<int>(floor((g / img_pitch) * yt + 0.5) + px_height_img * 0.5);
//            nz = static_cast<int>(floor((zt - 1.0 / max_d) / (1.0 / min_d - 1.0 / max_d) * (num_z_level - 1) + 0.5));
//
//            //beta[k][0] = (g / img_pitch) * xt - (nx - px_width_img * 0.5);
//            //beta[k][1] = (g / img_pitch) * yt - (ny - px_height_img * 0.5);
//
//            //beta[k][0] = (g / img_pitch) * xt - floor((g / img_pitch) * xt);
//            //beta[k][1] = (g / img_pitch) * yt - floor((g / img_pitch) * yt);
//
//            //cout << "beta x:" << beta[k][0] << ", beta y:" << beta[k][1] << endl;
//
//            //cout << "round((g / img_pitch) * yt):" << floor(-4.1+0.5) << endl;
//
...
//
//
//        //insert_pixels(0, Hpin, std::ref(img_display), std::ref(pcd_box), std::ref(sp), std::ref(tp), std::ref(data));
//
...
//            threads.emplace_back(insert_pixels, startRow, endRow, std::ref(img_display), std::ref(pcd_box), std::ref(sp), std::ref(tp), std::ref(data));
//        }
//
//        for (auto& t : threads) {
...
//        stream << "./images/v11-7check_ImgDisplay_NumZLevel" << num_z_level << "_min_d" << min_d << "_max_d" << max_d << "_pitchTimes" << fixed << setprecision(3) << ptimes << "_boxSize" << box_size << ".png";
...
//
//        //    stream << "./images/imagePlane/v11-5_imagePlane_NumZLevel" << num_z_level << "_zi" << zi << "_min_d" << min_d << "_pitchTimes" << fixed << setprecision(2) << ptimes << ".png";
//        //    cout << "zi:" << zi << endl;
//
...
//
//        ostringstream stream;
//        stream << "./images/True_pdBase_v11-7check_diff_img.png";
//        cv::String filename = stream.str();
//

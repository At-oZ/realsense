...
//                //cout << "i:" << i << ", j:" << j << endl;
//
//                newx = (g * inv_d) * (tmp_pcd_x * 1000.0 - uo[i * Wpin + j]);
//                newy = (g * inv_d) * (tmp_pcd_y * 1000.0 - vo[i * Hpin + j]);
//
//                nx = static_cast<int>(floor((newx * inv_wp + 0.5) * N));
//                ny = static_cast<int>(floor((newy * inv_wp + 0.5) * N));
//
...
//        stream << "./images/True_wpBase_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";

...
///* 差分:計算の高速化の計算(pcd_boxからr, g, b, aへの分離) */
//
...
//}
//
//void insert_pixels(int start, int end, cv::Mat img_display, unsigned char*** red, unsigned char*** green, unsigned char*** blue, bool*** alpha, int*** pcd_box, double* sp, double* tp) {
//
...
//
//                                //img_display.at<cv::Vec3b>(startv + m, startu + n)[2] = static_cast<int>(floor((tmp_pcd_color - 256 * 256 * 256) / (256 * 256)));
//                                //img_display.at<cv::Vec3b>(startv + m, startu + n)[1] = static_cast<int>(floor((tmp_pcd_color - 256 * 256 * 256) / 256 - img_display.at<cv::Vec3b>(startv + m, startu + n)[2] * 256));
//                                //img_display.at<cv::Vec3b>(startv + m, startu + n)[0] = static_cast<int>(floor((tmp_pcd_color - 256 * 256 * 256) - (img_display.at<cv::Vec3b>(startv + m, startu + n)[2] * 256 - img_display.at<cv::Vec3b>(startv + m, startu + n)[1]) * 256));
...
//            threads.emplace_back(insert_pixels, startRow, endRow, img_display, red, green, blue, alpha, pcd_box, sp, tp);
//        }
//
//        for (auto& t : threads) {
...
//        stream << "./images/prop-v1_v1-2_check_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";

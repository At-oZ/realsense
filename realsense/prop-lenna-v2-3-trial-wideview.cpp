...
//{
//
//    std::cout << "prop-v2-3-trial-wideview" << endl;
//
...
//
//    // 観察者のパラメータ
//    double observe_z = 500.0; // 観察者と表示系の距離
//    double observe_range = (observe_z + gap) / gap * pinhole_pitch; // 視域
//
//    // 要素画像のパラメータ
//    int element_image_px = static_cast<int>(floor(gap / observe_z * observe_range / display_pixel_pitch)); // 要素画像の解像度
//    int display_px = element_image_px * num_pinhole; // 各軸方向の表示画像の解像度
//    double interval_element_image = gap / observe_z * observe_range; // 要素画像の物理的間隔(mm)
//
...
//
//    // 表示系用カメラアレイのパラメータ
//    double camera_array_range = gap / observe_z * (pinhole_array_size + observe_range); // 要素カメラの画角
...
//        if (!interpolation) {
//
//            std::cout << "no interpolation" << endl;
...
//            threads.emplace_back(insert_pixels, startRow, endRow, each_camera_px, num_pinhole, num_z_level, boxel_cam_width_px, boxel_cam_height_px, std::ref(img_camera_array), std::ref(red), std::ref(green), std::ref(blue), std::ref(alpha), std::ref(nx), std::ref(ny));
//        }
//
//        for (auto& t : threads) {
...
//        for (int i = 0; i < num_pinhole; ++i) {
//
//            offsetv = static_cast<int>(floor(gap / observe_z * pinhole_pitch / display_pixel_pitch * i + 0.5));
//
//            for (int j = 0; j < num_pinhole; ++j) {
//
//                offsetu = static_cast<int>(floor(gap / observe_z * pinhole_pitch / display_pixel_pitch * j + 0.5));
//

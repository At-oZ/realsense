...
///* v3との差分：中央に画角が広く焦点距離がギャップに等しいカメラ(基準カメラ)を配置し、そのカメラで撮影した画像をもとに視差を用いて要素画像群を埋める */
//
...
//}
//
//void insert_pixels(int WIDTH, int HEIGHT, int Wpin, int Hpin, int N, double g, double wp, double pd, double intv, int starty, int endy, cv::Mat img, cv::Mat img_display, double** val_z) {
//
//    int nx, ny;
//    int startv, startu;
...
//        stream << "mac_v4_3_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png"; // �����_�ȉ�2���Ő؂�̂�
...
//    }
//
//    double ave = sum * 1.0 / 100;
//    cout << "Avarage Time:" << ave <<  "ms" << endl;
//    // free(elem_img);
//    // free(uo);
//    // free(vo);
//    for (int i = 0; i < N * range; i++) {

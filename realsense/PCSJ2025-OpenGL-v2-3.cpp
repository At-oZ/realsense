...
int main(void) {
	...
	// 今フレームで触れた画素を重複なく記録するフラグとリスト
	std::vector<unsigned char> dirtyFlag(TOTAL_PIXELS, 0u);
	std::vector<int> dirtyIndices;      // 前フレームで触れた画素（次フレーム先頭でゼロクリア対象）
	std::vector<int> nextDirtyIndices;  // 今フレームで触れた画素（フレーム末尾でスワップ）
	...
	while (!glfwWindowShouldClose(gridWin))
	{
		...
		if (firstFrame) {
			// 初回は全域クリア
			const size_t total = TOTAL_PIXELS;
			std::memset(red, 0, total * sizeof(unsigned int));
			std::memset(green, 0, total * sizeof(unsigned int));
			std::memset(blue, 0, total * sizeof(unsigned int));
			std::memset(numPoints, 0, total * sizeof(unsigned int));
			firstFrame = false;
		}
		else {
			// 前フレームで触れた画素だけを並列に0クリア
			const size_t N = dirtyIndices.size();
			if (N > 0) {
				const size_t chunk = (N + NUM_THREADS - 1) / NUM_THREADS; // ceil
				for (int i = 0; i < NUM_THREADS; ++i) {
					const size_t begin = size_t(i) * chunk;
					if (begin >= N) break;
			...
			// 使い終わったので空にしておく（この後に nextDirtyIndices とスワップ）
			dirtyIndices.clear();
		}
				...
				for (int m = -HALF_SEARCH_BOX_SIZE; m <= HALF_SEARCH_BOX_SIZE; ++m) {
					for (int n = -HALF_SEARCH_BOX_SIZE; n <= HALF_SEARCH_BOX_SIZE; ++n) {
						...
						if (0 <= x && x < Z_PLANE_IMG_PX_X && 0 <= y && y < Z_PLANE_IMG_PX_Y) {
							...
							// 初回アクセス時のみdirty登録（重複防止）
							if (dirtyFlag[idx] == 0u) {
								dirtyFlag[idx] = 1u;
								nextDirtyIndices.push_back(idx);
							}
		...
		// 今フレームで触れた画素インデックスを次フレームのクリア対象に
	...
	cout << "平均CPU実行時間: " << sum_cpu_time / (numFrame - 1) << " ms" << endl;
	cout << "平均GPU実行時間: " << sum_gpu_time / (numFrame - 1) << " ms" << endl;

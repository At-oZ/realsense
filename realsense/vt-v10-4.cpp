...
///* 差分：各要素画像の各画素を基準にとり, 視差を用いて計算を簡略化 */
//
...
//}
//
//void insert_pixels(int start, int end, int** img_display, int* pcd_box) {
//
...
//                        if (fabs(u_img) * 2 < W_img && fabs(v_img) * 2 < H_img) {
//
//                            // sum++;
//
//                            // イメージプレーン上の注目画素の画素番号
//                            x_img = static_cast<int>(floor((u_img + W_img * 0.5) / (W_img * inv_WIDTH)));
//                            y_img = static_cast<int>(floor((v_img + H_img * 0.5) / (H_img * inv_HEIGHT)));
//

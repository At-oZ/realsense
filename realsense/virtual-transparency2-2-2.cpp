...
//using namespace std;
//
//void insert_pixels(int WIDTH, int HEIGHT, int Wpin, int N, int D, double g, double wp, double inv_wp, double intv, int starty, int endy, double* uo, double* vo, rs2::points points, cv::Mat color_image, cv::Mat img_display, double* val_z) {
//
...
//        tmp_pcd1 = verts[k].y;
//        tmp_pcd2 = verts[k].z;
//        tmp_pcd3 = color_image.at<cv::Vec3b>(static_cast<int>(k * inv_WIDTH), (k % WIDTH))[0];
//        tmp_pcd4 = color_image.at<cv::Vec3b>(static_cast<int>(k * inv_WIDTH), (k % WIDTH))[1];
//        tmp_pcd5 = color_image.at<cv::Vec3b>(static_cast<int>(k * inv_WIDTH), (k % WIDTH))[2];
//
...
//        //    //cout << "i:" << i << endl;
//
//        //    raw_pointclouddata[i * 6] = verts[i].x;
//        //    raw_pointclouddata[i * 6 + 1] = verts[i].y;
//        //    raw_pointclouddata[i * 6 + 2] = verts[i].z;
//        //    raw_pointclouddata[i * 6 + 3] = color_image.at<cv::Vec3b>(static_cast<int>(i * inv_WIDTH), (i % WIDTH))[0];
//        //    raw_pointclouddata[i * 6 + 4] = color_image.at<cv::Vec3b>(static_cast<int>(i * inv_WIDTH), (i % WIDTH))[1];
//        //    raw_pointclouddata[i * 6 + 5] = color_image.at<cv::Vec3b>(static_cast<int>(i * inv_WIDTH), (i % WIDTH))[2];
//
//        //}
//
...
//            threads.emplace_back(insert_pixels, WIDTH, HEIGHT, Wpin, N, D, g, wp, inv_wp, intv, startRow, endRow, uo, vo, std::ref(points), std::ref(color_image), std::ref(img_display), val_z);
...
//        stream << "v10_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png"; // 小数点以下2桁で切り捨て

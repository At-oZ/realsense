...
//
//                    int idx_subz = 0;
//                    for (double subz = 550.0; subz <= 550.0; subz += 100) {
//
//
//                        // 観察者のパラメータ
...
//                        double focal_length = 13; // ギャップ(zo_min / (3 * nph - 1))
...
//                        double subject_size = 204.0; // 被写体のサイズ(拡大する場合 * (subz + zo_min) / zo_minを追加)
...
//                        //stream << "D:/EvacuatedStorage/prop-reconstruction/IE-prop-wideview-v1/prop-wideview-v1-pepper_f" << std::fixed << std::setprecision(4) << focal_length << "_subsize" << std::fixed << std::setprecision(2) << subject_size << "_zi" << (int)subz << ".png";

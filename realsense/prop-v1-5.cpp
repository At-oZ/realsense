...
///* 差分:計算の高速化の検討(v1-4の変更点 & double から float へ変更)→217msから228msに増加(?) */
//
...
//float g = 4.0, wp = 1.25, a = 0.125;                                                       // ギャップ、ピッチ、ピンホール幅
//float ld = 13.4 * 25.4;                                                                    // ディスプレイサイズ
//int pw = 3840, ph = 2400;                                                                   // ディスプレイの縦横の解像度
//float pd = ld / sqrtf(pw * pw + ph * ph);                                                  // 画素ピッチ
...
//float inv_wp = 1.0 / wp;         // ピンホールピッチの逆数
//float inv_WIDTH = 1.0 / WIDTH;   // WIDTHの逆数
//float inv_pd = 1.0 / pd;         // pdの逆数
//
//// パラメータ
//int num_z_level = 65; // イメージプレーンの層数
//float ptimes = 1; // イメージプレーンの画素ピッチの係数(1でディスプレイの画素ピッチと同じ)
//int box_size = 1; // 探索範囲
//
//int Ddash = 250;
//float coef = (float)num_z_level * (float)Ddash;
//float inv_coef = 1.0 / coef;
//
//float img_pitch = ptimes * pd;
//
...
//int fileread(float* test, string fname) {
//    ifstream fin(fname);
//    if (!fin) {
...
//}
//
//void insert_pixels(int start, int end, cv::Mat img_display, unsigned char*** red, unsigned char*** green, unsigned char*** blue, bool*** alpha, float* sp, float* tp) {
//
...
//    int startu, startv;
//    float u, v, xt, yt, zt;
//    float xt2, yt2, zt2, bxt, byt, bzt, dxt, dyt, dzt;
//    float tmp_sp, tmp_tp;
//
//    // 各要素画像の各画素ごとに
//    for (int m = start; m < end; m++) {
//
//        v = ((float)m - (float)(N - 1) * 0.5) * pd;
//
//        for (int n = 0; n < N; n++) {
//
//            u = ((float)n - (float)(N - 1) * 0.5) * pd;
//
//            // 各要素カメラごとに
...
//
//                    zt = (float)(num_z_level - 1) * inv_coef;
//                    xt = tmp_sp * zt + u / g;
//                    yt = tmp_tp * zt + v / g;
//
//                    // 各奥行きレベルごとに(手前から)
...
//                            if (alpha[nz][ny][nx]) {
//
//                                img_display.at<cv::Vec3b>(startv + m, startu + n)[2] = static_cast<int>(red[nz][ny][nx]);
//                                img_display.at<cv::Vec3b>(startv + m, startu + n)[1] = static_cast<int>(green[nz][ny][nx]);
//                                img_display.at<cv::Vec3b>(startv + m, startu + n)[0] = static_cast<int>(blue[nz][ny][nx]);
...
//}
//
//float getPSNR(const Mat& I1, const Mat& I2) {
//    Mat s1;
...
//
//    float sse = s.val[0] + s.val[1] + s.val[2]; // Sum channels
//
...
//    float* sp;
//    float* tp;
//    sp = (float*)malloc(sizeof(float) * Hpin * Wpin);
//    tp = (float*)malloc(sizeof(float) * Hpin * Wpin);
//    for (int i = 0; i < Hpin; i++) {
//        for (int j = 0; j < Wpin; j++) {
//
...
//    val_z = (float***)malloc(sizeof(float**) * num_z_level);
//
...
//        val_z[i] = (float**)malloc(sizeof(float*) * px_height_img);
//
...
//            val_z[i][j] = (float*)malloc(sizeof(float) * px_width_img);
//
//        }
//    }
//
...
//        //}
//
//        float inv_d, tmp_pcd_x, tmp_pcd_y, tmp_pcd_z, tmp_pcd_b, tmp_pcd_g, tmp_pcd_r;
//        float xt, yt, zt;
//        int nx, ny, nz;
//        int pcd_count = 0;
//
...
//            tmp_pcd_z = data[k][2] * 1000.0 + (float)D;
//            tmp_pcd_b = data[k][3];
//            tmp_pcd_g = data[k][4];
//            tmp_pcd_r = data[k][5];
//
...
//        stream << "./images/prop-v1-5_ImgDisplay_NumZLevel" << num_z_level << "_Ddash" << Ddash << "_pitchTimes" << fixed << setprecision(2) << ptimes << "_boxSize" << box_size << ".png";
...
//    //    }
//
//    //    float psnrValue = getPSNR(img1, img2);

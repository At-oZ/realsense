...
//}
//
//void insert_pixels(int start, int end, double uo_first, double vo_first, double* uu, double* vv, double* zz, double* coef, double* W_img, double* H_img, int** img_display, int* pcd_box) {
//
//    double u, v, z, u_img, v_img, px_range, box_size, W_img, H_img, coef, uo, vo;
//    int x_img, y_img, startu, startv, tmp_pcd_color, sum_zi;
//    int alpha = 256 * 256 * 256;
//    // bool flag1 = false;
//    // bool flag2 = false;
//
//    // sum_zi = 0;
//    // double uo_first = (- static_cast<int>(floor((Wpin - 1) * 0.5)) + 0.5) * wp;
//    // double vo_first = (- static_cast<int>(floor((Hpin - 1) * 0.5)) + 0.5) * wp;
//
//    // 各要素画像の各画素ごとに
...
//
//                        // 奥行きレベルにおける注目画素の位置
//                        // u_img = coef[zi] * (u - uo) + uo;
//                        // v_img = coef[zi] * (v - vo) + vo;
//
...
//                        if (fabs(u_img) * 2 < W_img[zi] && fabs(v_img) * 2 < H_img[zi]) {
//
//                            // sum++;
//
//                            // イメージプレーン上の注目画素の画素番号
//                            x_img = static_cast<int>(floor((u_img + W_img[zi] * 0.5) / (W_img[zi] * inv_WIDTH)));
//                            y_img = static_cast<int>(floor((v_img + H_img[zi] * 0.5) / (H_img[zi] * inv_HEIGHT)));
//
...
//                            // 注目範囲の大きさと画素の個数
//                            // px_range = coef * pd;
//                            // box_size = static_cast<int>(round(px_range / (W_img * inv_WIDTH)));
//
//                            // cout << "pixel range:" << px_range <<  ", box size:" << box_size << endl;
//
//                            tmp_pcd_color = pcd_box[(y_img * WIDTH + x_img) * num_z_level + zi];
...
//    // }
//    int* pcd_box;
//    pcd_box = (int*)malloc(sizeof(int) * HEIGHT * WIDTH * num_z_level);
//    
//    // // 視点パラメータ(mm)
//    // double** uo;
//    // double** vo;
//    // uo = (double*)malloc(sizeof(double) * Hpin);
//    // vo = (double*)malloc(sizeof(double) * Hpin);
//    // for (int i = 0; i < Hpin; i++) {
//    //     uo[i] = (double*)malloc(sizeof(double) * Wpin);
//    //     vo[i] = (double*)malloc(sizeof(double) * Wpin);
//    // }
//
//    // for (int i = 0; i < Hpin; i++) {
//    //     for (int j = 0; j < Wpin; j++) {
...
//    for (int i = 0; i < N; i++) {
//            u[i] = (i + 0.5) * pd + uo_first - N * 0.5 * pd;
//            v[i] = (i + 0.5) * pd + vo_first - N * 0.5 * pd;
//    }
//
//    double** u_img;
//    double** v_img;
//    u_img = (double**)malloc(sizeof(double*) * num_z_level);
//    v_img = (double**)malloc(sizeof(double*) * num_z_level);
//    for (int i = 0; i < num_z_level; i++) {
//        u_img[i] = (double*)malloc(sizeof(double) * N);
//        v_img[i] = (double*)malloc(sizeof(double) * N);
//    }
//    for (int i = 0; i < num_z_level; i++) {
//        for (int j = 0; j < N; j++) {
//            u_img[i][j] = coef[i] *  + uo;
//            v_img[i][j] = coef[i] * (v[j] - vo) + vo;
//        }
//    }
//
//    double* z;
...
//    double* W_img;
//    double* H_img;
//    z = (double*)malloc(sizeof(double) * num_z_level);
//    coef = (double*)malloc(sizeof(double) * num_z_level);
//    W_img = (double*)malloc(sizeof(double) * num_z_level);
//    H_img = (double*)malloc(sizeof(double) * num_z_level);
//    for (int i = 0; i < num_z_level; i++) {
//        z[i] = pow(base, i + 1);
//        coef[i] = (z[i] + D) / g;
//        W_img[i] = 2 * z[i] * Htan;
...
//                tmp_pcd_z = data[k][2];
//
//                z_level = floor(log10(tmp_pcd_z * 1000) / log10(base));
...
//
//                    // cout << "in!" << endl;
//                    pcd_box[(y_img * WIDTH + x_img) * num_z_level + z_level] = static_cast<unsigned char>(data[k][3]) + static_cast<unsigned char>(data[k][4]) * 256 + static_cast<unsigned char>(data[k][5]) * 256 * 256 + 256 * 256 * 256;
//                }
//            }
//        }
//
...
//            threads.emplace_back(insert_pixels, startRow, endRow, std::ref(img_display), std::ref(pcd_box));
//        }
//
//        for (auto& t : threads) {
...
//        stream << "mac_v10-4_img_display_g" << g << "_wp" << fixed << setprecision(1) << wp << "_pd" << fixed << setprecision(3) << pd << "_D" << D << ".png";
//

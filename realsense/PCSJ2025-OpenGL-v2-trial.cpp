...
//
//		//	// 連続走査用ポインタ（連続でない場合は未使用）
//		//	const cv::Vec3b* pixPtr = cont ? color_image.ptr<cv::Vec3b>(0) : nullptr;
//
//		//	// RealSenseの頂点配列は i=0..(W*H-1) で1点ずつアクセスする
//		//	const rs2::vertex* vtx = verts; // points.get_vertices()
//
//		//	for (int i = 0; i < W * H; ++i)
//		//	{
//		//		// 頂点（x,y,z）
...
//		//		float tmp_pcd_y = -v.y;
//		//		float tmp_pcd_z = v.z;
//		//		if (tmp_pcd_z <= 0.0f) continue; // 無効深度の早期スキップ
//
//		//		// 画素色（BGR8）を参照で取得（コピーを避ける）
//		//		unsigned char tmp_pcd_b, tmp_pcd_g, tmp_pcd_r;
//
//		//		if (cont) {
//		//			// 1次元インデックスでそのまま取得
//		//			const cv::Vec3b& c = pixPtr[i];
//		//			tmp_pcd_b = c[2]; // B
//		//			tmp_pcd_g = c[1]; // G
//		//			tmp_pcd_r = c[0]; // R
//		//		}
...
//		//			// 行ポインタから参照で取得
//		//			const int row = i / W;
//		//			const int col = i - row * W; // i % W より速い
//		//			const cv::Vec3b* rowp = color_image.ptr<cv::Vec3b>(row);
//		//			const cv::Vec3b& c = rowp[col];
//		//			tmp_pcd_b = c[2];
//		//			tmp_pcd_g = c[1];
//		//			tmp_pcd_r = c[0];
//		//		}
//
//		//		// 幾何 → 量子化
//		//		const float tmp_zt = 1.0f / tmp_pcd_z;
//		//		const float tmp_xt = tmp_pcd_x * tmp_zt;
//		//		const float tmp_yt = tmp_pcd_y * tmp_zt;
//
//		//		int tmp_nx = (int)lroundf(F_OVER_Z_PLANE_PITCH * tmp_xt) + HALF_Z_PLANE_IMG_PX_X;
//		//		int tmp_ny = (int)lroundf(F_OVER_Z_PLANE_PITCH * tmp_yt) + HALF_Z_PLANE_IMG_PX_Y;
//		//		int tmp_nz = (int)std::floor(COEF_TRANS * tmp_zt + 0.5f);
//
//		//		if (0 <= tmp_nz && tmp_nz < NUM_Z_PLANE) {
//		//			for (int m = -HALF_SEARCH_BOX_SIZE; m <= HALF_SEARCH_BOX_SIZE; ++m) {
//		//				for (int n = -HALF_SEARCH_BOX_SIZE; n <= HALF_SEARCH_BOX_SIZE; ++n) {
...
//		//						const int idx = (tmp_nz * Z_PLANE_IMG_PX_Y + y) * Z_PLANE_IMG_PX_X + x;
//		//						blue[idx] += (unsigned int)tmp_pcd_b;
//		//						green[idx] += (unsigned int)tmp_pcd_g;
//		//						red[idx] += (unsigned int)tmp_pcd_r;
//		//						numPoints[idx]++;
//		//					}
//		//				}
//		//			}

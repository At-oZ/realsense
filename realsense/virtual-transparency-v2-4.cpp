...
//
//    std::vector<double> inv_pcd_z(pointclouddata.size());
//    for (int k = startk; k < pointclouddata.size(); k++) {
//        inv_pcd_z[k] = 1.0 / pointclouddata[k][2];
//    }
//
//
...
//
//            std::vector<std::vector<double>> converted_pointclouddata(pointclouddata.size(), std::vector<double>(6));
//
//            // int型でインクリメント
...
//                    for (int k = startk; k < pointclouddata.size(); k++) {
//
//                        inv_delta = g * inv_pd * inv_pcd_z[k];
//                        converted_pointclouddata[k][0] = (pointclouddata[k][0] - (x + s)) * inv_delta;
//                        converted_pointclouddata[k][1] = (pointclouddata[k][1] - (y + t)) * inv_delta;
//                        converted_pointclouddata[k][2] = pointclouddata[k][2];
//                        converted_pointclouddata[k][3] = pointclouddata[k][3];
//                        converted_pointclouddata[k][4] = pointclouddata[k][4];
//                        converted_pointclouddata[k][5] = pointclouddata[k][5];
//                    }
//
...
//
//                    // 領域内のみコピー
//                    std::copy_if(converted_pointclouddata.begin(), converted_pointclouddata.end(), std::back_inserter(cliped_pointclouddata), [](const std::vector<double>& row) {
//                        return -0.5 <= row[0] && row[0] <= 0.5 && -0.5 <= row[1] && row[1] <= 0.5;
//                    });
//                    
//                    //std::cout << "size of cliped_pcd" << cliped_pointclouddata.size() << std::endl;
//

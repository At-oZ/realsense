...
//
//// パラメータ
//int num_z_level = 1000; // イメージプレーンの層数
//double ptimes = 0.5; // イメージプレーンの画素ピッチの係数(1でディスプレイの画素ピッチと同じ)
//int box_size = 3; // 探索範囲
//
...
//double coef = 1.0 / (double)(num_z_level - 1) * (1.0 / min_d - 1.0 / max_d);
//
...
//        v = wp / N * ((double)(m - N * 0.5) + 0.5);
//
//        for (int n = 0; n < N; n++) {
//
//            //u = ((double)n - (double)(N - 1) * 0.5) * pd;
//            u = wp / N * ((double)(n - N * 0.5) + 0.5);
//
//            // 各要素カメラごとに
...
//                    //tmp_tp = tp[i * Wpin + j];
//
//                    //xt = u / g + tmp_sp * (num_z_level - 1) * coef + 1.0 / max_d;
//                    //yt = v / g + tmp_tp * (num_z_level - 1) * coef + 1.0 / max_d;
//                    //zt = (num_z_level - 1) * coef + 1.0 / max_d;
//
//                    // 各奥行きレベルごとに(手前から)
...
//                            }
//                            else {
//                                for (int s = 1; s < static_cast<int>(floor((box_size - 1) * 0.5 + 0.5)); s++) {
//
//                                    // 上辺
...
//        stream << "./images/v11-7_wpBase_ImgDisplay_NumZLevel" << num_z_level << "_min_d" << min_d << "_max_d" << max_d << "_pitchTimes" << fixed << setprecision(3) << ptimes << "_boxSize" << box_size << ".png";
...
//
//    //    ostringstream stream;
//    //    stream << "./images/True_pdBase_v11-7_diff_img.png";
//    //    cv::String filename = stream.str();
//
